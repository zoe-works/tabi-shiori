        ...
        <div class="form-content fade-in" style="animation-delay: 0.1s;">
          ...
          <div class="card mb-md" id="destinations-container">
            ...
            <div id="destinations-list">
              ${t.destinations.map(n=>Zd(n,{country:I("countryPlaceholder"),city:I("cityPlaceholder")})).join("")}
            </div>
...
`).map(u=>u.trim()).filter(u=>u);_s[o].items=l});try{await Kg(n.uid,{checklistMaster:_s}),r.textContent=I("savedExclamation")||"保存しました！",setTimeout(()=>{r.disabled=!1,r.textContent=I("saveBtn")||"保存する"},2e3)}catch(i){alert((I("saveFailed")||"保存に失敗しました: ")+i.message),r.disabled=!1,r.textContent=I("saveBtn")||"保存する"}})}};function YS(r={}){const{immediate:e=!1,onNeedRefresh:t,onOfflineReady:n,onRegistered:s,onRegisteredSW:i,onRegisterError:o}=r;let c,l;const u=async(p=!0)=>{await l};async function d(){if("serviceWorker"in navigator){if(c=await Es(async()=>{const{Workbox:p}=await import("./workbox-window.prod.es5-BqEJf4Xk.js");return{Workbox:p}},[]).then(({Workbox:p})=>new p("/tabi-shiori/sw.js",{scope:"/tabi-shiori/",type:"classic"})).catch(p=>{o==null||o(p)}),!c)return;c.addEventListener("activated",p=>{(p.isUpdate||p.isExternal)&&window.location.reload()}),c.addEventListener("installed",p=>{p.isUpdate||n==null||n()}),c.register({immediate:e}).then(p=>{i?i("/tabi-shiori/sw.js",p):s==null||s(p)}).catch(p=>{o==null||o(p)})}}return l=d(),u}tt("/",NS);tt("/flashcard",LS);tt("/checklist",US);tt("/schedule",qS);tt("/research",zS);tt("/budget",GS);tt("/emergency",HS);tt("/omiyage",KS);tt("/trip/new",ey);tt("/trip/edit",ey);tt("/settings",JS);function XS(){const r=document.getElementById("app");r.innerHTML=`
  ...
  `}async function ZS(){var n,s,i;"serviceWorker"in navigator&&YS({immediate:!0}),XS(),document.querySelectorAll(".nav-item").forEach(o=>{o.addEventListener("click",c=>{c.preventDefault();const l=o.getAttribute("data-route");le(l)})});const r=document.getElementById("drawer-overlay");document.getElementById("btn-menu").addEventListener("click",()=>{r.classList.add("active")}),document.getElementById("btn-share").addEventListener("click",async()=>{const o=te();if(!o.currentTrip)return;let c=o.currentTrip.shareId;if(!c){const l=document.getElementById("btn-share"),u=l.innerHTML;l.innerHTML="⏳";try{c=await createShareLink(o.currentTrip.ownerId||o.user.uid,o.currentTrip.id,""),await updateTrip(o.currentTrip.ownerId||o.user.uid,o.currentTrip.id,{shareId:c}),Ye({currentTrip:{...o.currentTrip,shareId:c}})}catch{alert("共有コードの生成に失敗しました。"),l.innerHTML=u;return}l.innerHTML=u}navigator.clipboard.writeText(c).then(()=>{let l=document.getElementById("share-toast");l||(l=document.createElement("div"),l.id="share-toast",l.style.position="fixed",l.style.bottom="80px",l.style.left="50%",l.style.transform="translateX(-50%)",l.style.background="var(--color-primary)",l.style.color="white",l.style.padding="12px 24px",l.style.borderRadius="24px",l.style.boxShadow="0 4px 12px rgba(0,0,0,0.15)",l.style.zIndex="9999",l.style.transition="opacity 0.3s ease",l.style.fontWeight="bold",document.body.appendChild(l)),l.textContent="共有コードをコピーしました！",l.style.opacity="1",setTimeout(()=>{l.style.opacity="0"},3e3)}).catch(()=>{alert("共有コード: "+c)})}),[r].forEach(o=>{o.addEventListener("click",c=>{c.target===o&&o.classList.remove("active")})}),document.querySelectorAll(".drawer-item[data-route]").forEach(o=>{o.addEventListener("click",()=>{const c=o.getAttribute("data-route");r.classList.remove("active"),le(c)})}),(n=document.getElementById("btn-drawer-google"))==null||n.addEventListener("click",async()=>{const{user:o}=te();if(!(o&&!o.isAnonymous))try{o&&o.isAnonymous?await Jl():await Ql(),window.location.reload()}catch(c){c.code!=="auth/popup-closed-by-user"&&c.code!=="auth/cancelled-popup-request"&&alert("ログインに失敗しました: "+c.message)}}),(s=document.getElementById("btn-drawer-logout"))==null||s.addEventListener("click",async()=>{try{await Vg(),window.location.reload()}catch(o){alert("ログアウトに失敗しました: "+o.message)}}),(i=document.getElementById("btn-drawer-portal"))==null||i.addEventListener("click",()=>{Ye({currentTripId:null,currentTrip:null}),document.getElementById("drawer-overlay").classList.remove("active"),le("/",!0,!0)}),document.querySelectorAll(".btn-lang").forEach(o=>{o.addEventListener("click",()=>{const c=o.getAttribute("data-lang");ef(c),window.location.reload()})});try{const o=await xg();Ye({user:o});let c=[];o&&(c=await ko(o.uid)),Ye({trips:c}),Ye({currentTripId:null,currentTrip:null}),ty(c)}catch(o){console.error("Initial data load failed:",o)}const e=document.getElementById("loading-screen");e&&e.classList.add("hidden");const t=window.location.hash.replace("#","")||"/";le(t,!1)}function ty(r){var n;const e=document.getElementById("drawer-trips");if(!e)return;const t=te();e.innerHTML=`
    <p class="drawer-section-title">${I("switchTrip")}</p>
    ${r.map(s=>`
