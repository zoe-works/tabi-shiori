  ...
  `}async function dA(){if(!ne().currentTrip){document.getElementById("cl-container").innerHTML=`<p class="empty-state">${_("noTripSelected")}</p>`;return}pA(),await Dm()}async function Dm(){const r=ne(),e=r.currentTrip;if(Yn=await $h(e.id),Yn.length===0){let t=lA;if(r.user){const n=await km(r.user.uid);n&&n.checklistMaster&&(t=n.checklistMaster)}for(const n of t)for(const s of n.items){const i={name:s,category:n.category,checked:!1,important:uA.includes(s),assignee:null};await Cm(e.id,i)}Yn=await $h(e.id)}fA()}async function fA(){const r=document.getElementById("cl-container");r.innerHTML="";for(const e of Object.keys(Yh)){const t=Yh[e],n=Yn.filter(l=>l.category===e);if(n.length===0)continue;const s=document.createElement("div");s.className="checklist-section";const i=document.createElement("div");i.className="checklist-header",i.style.backgroundColor=t.color,i.innerHTML=`<h3>${t.label}</h3><span class="toggle-icon">▼</span>`,i.addEventListener("click",()=>{const l=s.querySelector(".checklist-items");l.classList.toggle("hidden"),i.querySelector(".toggle-icon").textContent=l.classList.contains("hidden")?"▶":"▼"}),s.appendChild(i);const o=document.createElement("div");o.className="checklist-items";for(const l of n){l.checked;const h=document.createElement("div");h.className=`checklist-item ${l.checked?"checked":""}`;const f=await at(l.name)||l.name;h.innerHTML=`
   ...
   <button type="button" class="btn btn-secondary w-full mt-sm" id="em-cancel">${_("cancelBtn")||"キャンセル"}</button>
        ...
        `},async init(){var h;(h=document.getElementById("btn-back-emergency"))==null||h.addEventListener("click",()=>oe("/"));const e=ne().currentTripId,t=document.getElementById("emergency-main");if(!e){t.innerHTML=`
                ...
                `).join("");else{const f=Object.keys(rn()).map(async g=>{const I=n.filter(C=>C.category===g),S=I.map(async C=>{const U=await at(C.title)||C.title,j=C.info?await at(C.info)||C.info:"";return`
   ...
   <button type="button" class="btn btn-secondary w-full mt-sm" id="omi-cancel">${_("cancelBtn")||"キャンセル"}</button>
        ...
        `},async init(){var p;(p=document.getElementById("btn-back-omiyage"))==null||p.addEventListener("click",()=>oe("/"));const e=ne().currentTripId,t=document.getElementById("omiyage-main"),n=document.getElementById("omiyage-progress-container");if(!e){t.innerHTML=`
            ...
            `;const S={};for(const U of s){const j=U.recipientName;S[j]||(S[j]=[]),S[j].push(U)}const D=Object.keys(S).map(async U=>{const j=S[U],O=j.reduce((L,T)=>L+Number(T.budget||0),0),$=j.map(async L=>{const T=await at(L.itemName)||_("undecided")||"未定";return`
